...
// Constants.
const web3 = new Web3(window.web3.currentProvider);     // Using 'window.ethereum' less reliable.
...
async function connectWallet() {
    ...
    if (window.ethereum) {
        ...
        .then(() => {
            ...
            fadeElement($metamaskLogo);
            fadeElement($metamaskLogoGlow);
            fadeElement($metamaskText);
...
function restorePriorSession() {
    ...
    let priorPendingTx = localStorage.getItem('pendingTx');
    if (priorPendingTx != null) {
        ...
        .then((txData) => {
            ...
            if (txData.blockNumber != null) {
                ...
                if (localStorage.getItem('priorTx' + validChain) == null) {
                    localStorage.setItem('priorTx' + validChain,
                        pendingTxHash + '@' +
                        txData.blockNumber
                    );
                } else {
                    localStorage.setItem('priorTx' + validChain,
                        localStorage.getItem('priorTx' + validChain) + '@' + 
                        pendingTxHash + '@' + 
                        txData.blockNumber
                    );
                }
                ...
                localStorage.removeItem('txLog1');
                localStorage.removeItem('txLogTitle');
                localStorage.removeItem('txDetailsTitle');
                localStorage.removeItem('txDetails');
                pendingTxHash = '';
            // Else it is still pending.
                ...
                $txLog1.innerHTML = localStorage.getItem('txLog1');
                $txLogTitle.innerHTML = localStorage.getItem('txLogTitle');
                $txLogHeader.innerHTML = '';
                $txDetailsTitle.innerHTML = localStorage.getItem('txDetailsTitle');
                $txDetails.innerHTML = localStorage.getItem('txDetails');
...
document.getElementById('processTransaction').addEventListener('click', async () => {
    ...
    {
        displayNotification('Please check the invalid or empty inputs and try again.', 3000);
        return;
    }
    ...
    // This event fires when metamask sends the tx to the mempool.
    .on("sent", (sent) => {
...
export async function refreshAccountBalance() {
    ...
    .then( async (balance) => {
        tokenBalance = (new BN(balance / BigInt(10**tokenDecimals))).toString(); //await web3.utils.fromWei(bal, 'ether');
        if (displayBalance) {
            document.getElementById('tokenBalance').innerHTML = parseFloat(Number(tokenBalance).toFixed(9)) + ' ' + token + ' <span style="font-size: 0.9cqmax">(to send)</span>'; // parseFloat is to remove trailing zeros.
        } else {
...
async function processLatestBlock(blockNumber) {
    ...
    if (pendingTxHash.length == 0) {
        ...
        $txLogHeader.innerHTML = '<span style="text-decoration-line: underline;">FEE TYPE&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspGWEI&nbsp&nbsp&nbsp&nbsp&nbspTIMING</span>';
        ...
        let priorTxsString = localStorage.getItem('priorTx' + validChain);
        //console.log(priorTxsString.split('@'));
        ...
        /*let url = `${etherscanEndpoint}?module=account&action=txlist&address=${accounts[0]}&startblock=${blockNumberOnWalletLoad}&endblock=99999999&page=1&offset=10&sort=asc&apikey=${etherscanApiKey}`;
        ...
        .then((response) => {
                ...
                while (response.result[i] != undefined) {
                    txArray.push(buildPastTxString( response.result[i].hash, response.result[i].confirmations, linkUrlPrefix ));
                    txFound = true;
                    i += 2;
                }
                if (txFound) {
        ...
        .catch((err) => { genericErrHandler(err,'fetching past tx list', false); });*/
...
async function processNewPendingTx(txData) {
    ...
    localStorage.setItem('txLog1', $txLog1.innerHTML);
    localStorage.setItem('txLogTitle', $txLogTitle.innerHTML);
    localStorage.setItem('txDetailsTitle', $txDetailsTitle.innerHTML);
    localStorage.setItem('txDetails', $txDetails.innerHTML);
}
...
function processConfirmedTx(txData) {
    ...
    } else if (localStorage.getItem('priorTx' + validChain).indexOf(pendingTxHash) == -1) {
        localStorage.setItem('priorTx' + validChain,
            localStorage.getItem('priorTx' + validChain) + '@' + 
            pendingTxHash + '@' + 
            txData.blockNumber
    ...
    setTimeout(() => { processLatestBlock(currentBlockNumber); }, 5000);
}
...
function buildPastTxString(hash, confirmations, linkUrlPrefix) {
    ...
    if (confirmations.length > 0) { return hashStr + ', ' + confirmations + ' confirmations'; }
    else { return hashStr; }
}
 ...
 */
function fadeElement(elementRef) {
    let opacity = 1;
